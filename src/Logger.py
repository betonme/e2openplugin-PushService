...
class Logger(object):
	def __init__(self):
		self.instance = logging.getLogger("pushservice")
	...
	def reinit(self):
		...
		if not hasattr(config, "pushservice"):
			return
		
		if config.pushservice.log_shell.value:
		...
		if config.pushservice.log_write.value:
			fhandler = logging.FileHandler(config.pushservice.log_file.value)
	...
	def info(self, *args):
		...
		elif config.pushservice.log_shell.value:
			print strargs
	...
	def warning(self, *args):
		...
		if int(config.pushservice.popups_warning_timeout.value) != 0:
	...
	def error(self, *args):
		...
		if int(config.pushservice.popups_error_timeout.value) != 0:
			if currentThread().getName() == 'MainThread':
				AddPopup(
					strargs,
